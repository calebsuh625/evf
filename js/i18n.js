/**
 * i18n.js — English / Simplified Chinese.
 *
 * Bilingual from day one (principle 6), which means the dictionary is the
 * only place UI copy lives. A string hardcoded in a view is a bug: it is
 * text a student's parent cannot read.
 *
 * Keys are dotted paths. Missing keys fall back to English, then to the key
 * itself, and log once — a silently blank label is worse than an ugly one.
 */

import { loadLangPreference, saveLangPreference } from './store.js';

export const LANGS = Object.freeze(['en', 'zh']);

export const LOCALES = Object.freeze({ en: 'en-US', zh: 'zh-CN' });

const STRINGS = {
  en: {
    'app.title': 'PeerBridges 2.0',
    'lang.toggle': '中文',

    'nav.home': 'Home',
    'nav.tutors': 'Tutors',
    'nav.students': 'Students',
    'nav.sessions': 'Sessions',
    'nav.hours': 'Hours',
    'nav.settings': 'Settings',

    'footer.local': 'All data stays in this browser. Export to keep it.',
    'footer.tests': 'Run tests',

    'home.lede': 'One-on-one tutoring between US high school volunteers and students in mainland China. Everything runs in this browser — there is no server holding anyone\'s information.',
    'home.empty.title': 'Nothing loaded yet',
    'home.empty.body': 'Load the demo dataset to see how the app works, or import a program file you exported earlier.',
    'home.zone.here': 'Your time',
    'home.zone.students': 'Students',

    'students.title': 'Students',
    'log.title': 'Log a session',
    'hours.title': 'Hours',
    'hours.lede': 'Computed from logged sessions. Nobody enters an hour figure by hand.',
    'settings.title': 'Settings',
    'settings.lede': 'Language, time zones, and session defaults.',

    'data.import.drop': 'Choose a JSON file, or drop one here',
    'data.sample.action': 'Load sample data',
    'data.status.title': 'Currently loaded',
    'data.status.schema': 'Schema version',
    'data.status.cache': 'Browser cache',
    'data.status.exported': 'Last exported',
    'data.status.never': 'Never',
    'data.reset.title': 'Start over',
    'data.reset.body': 'Clears this browser\'s copy. Your exported files are untouched.',
    'data.reset.action': 'Clear this browser',
    'data.reset.confirm': 'Clear all data from this browser? Anything not exported is gone.',

    'data.status.pairings': 'Pairings (active)',
    'data.status.availability': 'Availability rows',

    'selftest.title': 'Time zone self-test',
    'selftest.lede': 'The timezone assertions, run in this browser, right now. This is the part of the app most likely to be wrong, so it is the part you can check.',
    'selftest.pass': 'All {total} checks passing',
    'selftest.fail': '{failed} of {total} checks failing',
    'selftest.ranIn': 'Ran in this browser in {ms} ms.',
    'selftest.rerun': 'Run again',
    'selftest.reran': 'Checks re-run.',
    'selftest.example': 'The case that matters most',
    'selftest.tutorSide': 'Tutor · US Pacific',
    'selftest.studentSide': 'Student · Beijing',
    'selftest.exampleNote': 'One instant ({utc}), named differently at each end.',
    'selftest.expected': 'Expected',
    'selftest.actual': 'Actual',
    'selftest.passed': 'passing',
    'selftest.failed': 'failing',
    'selftest.nFailing': '{failed} failing',
    'selftest.footnote': 'These are the same assertions the unit suite runs. For the full suite, see',
    'footer.selftest': 'Time zone self-test',
    'role.label': 'Viewing as',
    'role.admin': 'Coordinator',
    'role.tutors': 'Tutors',
    'role.needTutor.title': 'Pick a tutor first',
    'role.needTutor.body': 'These screens show one tutor\'s own teaching. Choose who you are from the picker at the top right.',
    'role.needTutor.action': 'Back to home',

    'tutor.nav.home': 'My teaching',
    'tutor.nav.hours': 'My hours',
    'tutor.nav.availability': 'My availability',

    'tutor.home.greeting': 'Hi {name}',
    'tutor.home.lede': 'Your next class, your students, and your hours.',
    'tutor.home.noStudents.title': 'No students yet',
    'tutor.home.noStudents.body': 'Once you are paired with a student they will show up here, along with your next class and everything you have covered together.',
    'tutor.home.setAvailability': 'Set my availability',

    'tutor.next.title': 'Next class',
    'tutor.next.none': 'Nothing scheduled. Set your availability and the coordinator can pair you.',
    'tutor.next.yourTime': 'Your time',
    'tutor.next.theirTime': 'Their time',
    'tutor.next.recurring': 'From your shared weekly window',
    'tutor.next.scheduled': 'Scheduled',
    'tutor.next.join': 'Open meeting link',
    'tutor.next.homework': 'Homework you set last time',
    'tutor.next.noHomework': 'No homework set last time.',
    'tutor.next.lastCovered': 'Last time you covered',
    'tutor.next.logIt': 'Log this class',

    'tutor.students.title': 'My students',
    'tutor.students.goals': 'Working on',
    'tutor.students.lastSession': 'Last session',
    'tutor.students.never': 'Not met yet',
    'tutor.students.sessions': '{count} sessions',
    'tutor.students.open': 'Open student page',
    'tutor.students.log': 'Log a session',

    'tutor.hours.title': 'My hours',
    'tutor.hours.lede': 'Computed from the classes you logged. Nothing here is typed in by hand.',
    'tutor.hours.basis': 'Every class you held counts as 2 hours — teaching, preparation and follow-up together. You are never asked how long anything took, and a class that ran short counts the same as one that ran long.',
    'tutor.hours.basisShort': '{count} classes × 2 hours',
    'tutor.hours.classTime': 'Class time',
    'tutor.hours.breakdownTitle': 'How this adds up',
    'tutor.cert.basis': 'Basis: this program credits each class held at a standard {hours} hours, covering preparation, the class itself and follow-up. Total = {count} classes × {hours} hours. Individual class lengths are not separately measured; each class listed below was logged by the volunteer at the time it took place.',
    'tutor.hours.credited': 'Credited',
    'tutor.hours.thisTerm': 'This term',
    'tutor.hours.allTime': 'All time',
    'tutor.hours.teaching': 'Teaching',
    'tutor.hours.prep': 'Prep',
    'tutor.hours.followup': 'Follow-up',
    'tutor.hours.total': 'Total',
    'tutor.hours.hoursShort': 'h',
    'tutor.hours.sessions': 'Sessions',
    'tutor.hours.students': 'Students',
    'tutor.hours.tableTitle': 'Every session',
    'tutor.hours.date': 'Date',
    'tutor.hours.student': 'Student',
    'tutor.hours.duration': 'Teaching',
    'tutor.hours.empty': 'No logged sessions yet. Once you log one it will appear here and count towards your hours.',
    'tutor.hours.export.title': 'Export for verification',
    'tutor.hours.export.body': 'A signed record of volunteer hours, formatted for NHS, the Congressional Award, and the President\'s Volunteer Service Award. Print it or save it as a PDF.',
    'tutor.hours.export.print': 'Print / save as PDF',
    'tutor.hours.export.csv': 'Download CSV',
    'tutor.hours.export.term': 'Period',
    'tutor.hours.export.allTime': 'All time',

    'tutor.cert.title': 'Volunteer Service Record',
    'tutor.cert.volunteer': 'Volunteer',
    'tutor.cert.organisation': 'Organisation',
    'tutor.cert.activity': 'Activity',
    'tutor.cert.activityBody': 'One-to-one online academic tutoring for students in mainland China. Sessions are held at weekends over video call.',
    'tutor.cert.period': 'Period',
    'tutor.cert.school': 'School',
    'tutor.cert.generated': 'Generated',
    'tutor.cert.summary': 'Summary of hours',
    'tutor.cert.detail': 'Session detail',
    'tutor.cert.verify': 'Verification',
    'tutor.cert.verifyBody': 'I confirm that the volunteer named above completed the hours recorded in this document.',
    'tutor.cert.supervisor': 'Supervisor name',
    'tutor.cert.signature': 'Signature',
    'tutor.cert.date': 'Date',
    'tutor.cert.note': 'Hours are derived from session records logged by the volunteer at the time of each session.',

    'tutor.log.title': 'Log a session',
    'tutor.log.with': 'with {name}',
    'tutor.log.happened': 'Did it happen?',
    'tutor.log.yes': 'Yes',
    'tutor.log.no': 'No',
    'tutor.log.duration': 'How long?',
    'tutor.log.prep': 'Prep',
    'tutor.log.followup': 'Follow-up',
    'tutor.log.credits': 'This counts as 2 hours.',
    'tutor.log.creditsNone': 'Nothing to log. This does not count against you.',
    'tutor.log.covered': 'What did you cover?',
    'tutor.log.homework': 'Homework set',
    'tutor.log.optional': 'optional',
    'tutor.log.save': 'Save',
    'tutor.log.saved': 'Saved. Thank you.',
    'tutor.log.total': 'Total {total}',
    'tutor.log.capped': 'Capped at 2 hours per session.',
    'tutor.log.minutes': '{n} min',
    'tutor.log.none': 'None',

    'tutor.nudge.title': 'Not logged yet',
    'tutor.nudge.body': 'These classes have been and gone. Log them whenever suits you.',
    'tutor.nudge.log': 'Log',

    'tutor.student.title': 'Student',
    'tutor.student.about': 'About',
    'tutor.student.level': 'English level',
    'tutor.student.goals': 'Goals',
    'tutor.student.interests': 'Interests',
    'tutor.student.grade': 'Grade',
    'tutor.student.timezone': 'Time zone',
    'tutor.student.pairedSince': 'Paired since',
    'tutor.student.handover': 'Picking up from here',
    'tutor.student.handoverBody': 'Written so somebody new can read this page and know where to start.',
    'tutor.student.history': 'Session history',
    'tutor.student.covered': 'Covered',
    'tutor.student.homework': 'Homework',
    'tutor.student.didNotHappen': 'Did not happen',
    'tutor.student.noHistory': 'No sessions logged yet.',
    'tutor.student.notYours': 'That student is not one of yours.',
    'tutor.student.totals': '{sessions} sessions · {hours}h together',

    'tutor.avail.title': 'My availability',
    'tutor.avail.lede': 'Only used when you are looking for a new student. It is not a commitment to be free every week.',
    'tutor.avail.accepting': 'Open to a new student',
    'tutor.avail.acceptingOn': 'You are open to being paired with a new student.',
    'tutor.avail.acceptingOff': 'You are not taking new students right now. Nothing else changes, and your current students are unaffected.',
    'tutor.avail.windows': 'Weekend windows',
    'tutor.avail.windowsBody': 'Your local time, with what that is for a student in China.',
    'tutor.avail.add': 'Add a window',
    'tutor.avail.remove': 'Remove',
    'tutor.avail.day': 'Day',
    'tutor.avail.from': 'From',
    'tutor.avail.to': 'To',
    'tutor.avail.theirs': 'In China',
    'tutor.avail.none': 'No windows set. Add one and the coordinator can pair you with a student who fits.',
    'tutor.avail.save': 'Save availability',
    'tutor.avail.saved': 'Availability saved.',
    'tutor.avail.overnight': 'crosses midnight',

    'weekday.0': 'Sunday',
    'weekday.1': 'Monday',
    'weekday.2': 'Tuesday',
    'weekday.3': 'Wednesday',
    'weekday.4': 'Thursday',
    'weekday.5': 'Friday',
    'weekday.6': 'Saturday',
    'role.students': 'Students',
    'role.guardians': 'Guardians',
    'role.guardianOf': 'Guardian of {name}',
    'role.needStudent.title': 'Pick a student first',
    'role.needStudent.body': 'This screen shows one student\'s own classes. Choose who you are from the picker at the top right.',

    'st.nav.home': 'My classes',
    'st.title': 'My classes',
    'st.greeting': 'Hi {name}',
    'st.tutor.title': 'My tutor',
    'st.tutor.teaches': 'Teaches',
    'st.tutor.contact': 'Contact',
    'st.tutor.none': 'You have not been paired with a tutor yet. Your coordinator is working on it.',
    'st.next.title': 'Next class',
    'st.next.none': 'No class scheduled yet.',
    'st.next.join': 'Join the class',
    'st.next.tutorTime': 'Your tutor\'s time',
    'st.homework.title': 'Homework',
    'st.homework.none': 'Nothing set right now.',
    'st.homework.from': 'Set on {date}',
    'st.history.title': 'Past classes',
    'st.history.homework': 'Homework',
    'st.history.none': 'No classes yet.',
    'st.history.didNotHappen': 'This class did not happen',
    'st.nothingRequired': 'There is nothing you need to fill in here.',

    'gd.contact.title': 'Guardian contact details',
    'gd.contact.body': 'Optional. Leave anything blank and it stays blank — the program will still work exactly the same.',
    'gd.contact.name': 'Your name',
    'gd.contact.wechat': 'WeChat',
    'gd.contact.email': 'Email',
    'gd.contact.save': 'Save',
    'gd.contact.saved': 'Saved.',
    'gd.contact.onlyYou': 'Only you can edit these.',

    'match.nav': 'Matching',
    'match.title': 'Matching',
    'match.lede': 'Suggestions only. Nothing is paired until you accept it.',
    'match.waiting.title': 'Students waiting',
    'match.waiting.none': 'Every active student has a tutor.',
    'match.blocked.title': 'Nobody can take these students yet',
    'match.blocked.body': 'Each one says what would have to change.',
    'match.idle.title': 'Volunteers with room and nobody to take',
    'match.idle.body': 'These tutors are free and willing. Worth recruiting a student who fits.',
    'match.stale.title': 'Pairings that no longer share any time',
    'match.stale.body': 'These are active but their availability has drifted apart. Nobody will mention it, so this is the only place it shows up.',
    'match.stale.noOverlap': 'No shared time',
    'match.stale.missing': 'One of them has not set any availability',
    'match.candidate': 'Suggestion {n}',
    'match.why': 'Why',
    'match.watch': 'Worth knowing',
    'match.score': 'Fit',
    'match.accept': 'Pair them',
    'match.accepted': 'Paired {student} with {tutor}.',
    'match.slotsOpen': '{remaining} of {total} places open',
    'match.nearest': 'Closest so far',
    'match.blockedBy': 'Blocked by',
    'match.counts': '{waiting} waiting · {matchable} matchable · {blocked} blocked',

    'match.part.morning': 'morning',
    'match.part.afternoon': 'afternoon',
    'match.part.evening': 'evening',

    'match.reason.overlap': '{hours} shared hours, {weekday} {part} Beijing time ({startTime}–{endTime})',
    'match.reason.goals': 'Both listed {list}',
    'match.reason.level': 'Tutor is comfortable at {level} level',
    'match.reason.interests': 'Both interested in {list}',
    'match.reason.capacity': 'Tutor has {remaining} of {total} places open',

    'match.weak.single-window': 'Only one shared window — fragile if either has to cancel',
    'match.weak.short-overlap': 'Only {minutes} minutes shared, so there is little room to move',
    'match.weak.goals-partial': 'Does not cover {list}',
    'match.weak.level-unstated': 'Level fit is unconfirmed — nobody has said which levels they take',
    'match.weak.no-shared-interests': 'No interests in common',
    'match.weak.no-shared-language': 'No shared language, and the student is a beginner',
    'match.weak.tutor-last-slot': 'This is the tutor\'s last place',

    'match.blocker.no-shared-time': 'No shared time',
    'match.blocker.no-shared-goal': 'Does not teach what the student asked for',
    'match.blocker.level-outside-comfort': 'Level outside what the tutor takes',
    'match.blocker.tutor-at-capacity': 'Tutor is full',
    'match.blocker.tutor-not-accepting': 'Tutor is not taking new students',
    'match.blocker.tutor-inactive': 'Tutor is on a break',
    'match.blocker.student-inactive': 'Student is inactive',

    'match.fix.no-tutors': 'There are no tutors on the roster yet.',
    'match.fix.matchable': 'This student is matchable.',
    'match.fix.one-tutor-full': 'One tutor fits this student well but has no places left. This needs another tutor with the same subject and hours, not more asking.',
    'match.fix.all-tutors-full': 'Every suitable tutor is at the maximum they set for themselves. This needs another tutor.',
    'match.fix.no-shared-time': 'Nobody shares a weekend window with this student. Ask them for one more slot, or recruit a tutor further west.',
    'match.fix.no-shared-goal': 'No available tutor covers what this student asked for. Recruit for that subject, or ask the student what else would help.',
    'match.fix.level-outside-comfort': 'No available tutor has said they take this English level. Worth asking whether someone would try it.',
    'match.fix.tutor-not-accepting': 'The tutors who fit are not taking new students right now.',
    'match.fix.tutor-inactive': 'The tutors who fit are on a break.',
    'match.fix.student-inactive': 'This student is marked inactive.',
    'admin.nav.overview': 'Overview',
    'admin.nav.attention': 'Needs attention',
    'admin.nav.roster': 'Roster',
    'admin.nav.export': 'Export',

    'admin.computed': 'Everything on these screens is computed from records people already keep. Nobody is asked to fill anything in so a number can appear here.',

    'admin.ov.title': 'Overview',
    'admin.ov.lede': 'The state of the program right now.',
    'admin.ov.activePairings': 'Active pairings',
    'admin.ov.tutors': 'Tutors',
    'admin.ov.students': 'Students',
    'admin.ov.paused': '{n} paused',
    'admin.ov.inactive': '{n} inactive',
    'admin.ov.waiting': '{n} waiting',
    'admin.ov.hours': 'Hours taught',
    'admin.ov.week': 'This week',
    'admin.ov.month': 'This month',
    'admin.ov.allTime': 'All time',
    'admin.ov.growth': 'Growth',
    'admin.ov.growthBody': 'People on the roster at the end of each month, and classes held.',
    'admin.ov.sessionsPerMonth': 'Classes held',
    'admin.ov.attentionCta': 'See what needs attention',

    'admin.att.title': 'Needs attention',
    'admin.att.lede': 'The things nobody will report on their own.',
    'admin.att.allClear': 'Nothing needs attention. Every active pairing has met recently, every student has a tutor, and no classes were missed this month.',
    'admin.att.quiet.title': 'Pairings that have gone quiet',
    'admin.att.quiet.body': 'No class logged for {weeks}+ weeks, longest first. Worth a friendly message to ask how it is going.',
    'admin.att.quiet.days': '{n} days',
    'admin.att.quiet.never': 'Never met',
    'admin.att.quiet.lastMet': 'Last met {date}',
    'admin.att.quiet.none': 'Every active pairing has met recently.',
    'admin.att.waiting.title': 'Students with no tutor',
    'admin.att.waiting.body': 'Active students with no active pairing.',
    'admin.att.waiting.none': 'Every active student has a tutor.',
    'admin.att.waiting.returning': 'Had a tutor before',
    'admin.att.waiting.new': 'New',
    'admin.att.capacity.title': 'Tutors with room',
    'admin.att.capacity.body': 'Willing and not yet full. An unused volunteer does not complain, they drift away.',
    'admin.att.capacity.none': 'No tutor currently has room.',
    'admin.att.capacity.open': '{remaining} of {capacity} open',
    'admin.att.misses.title': 'Classes that did not happen',
    'admin.att.misses.body': 'The last {days} days. One is normal; a run of them usually means something changed.',
    'admin.att.misses.none': 'No missed classes in the last {days} days.',
    'admin.att.contact': 'Contact',
    'admin.att.noContact': 'No contact details on file',
    'admin.att.openMatching': 'Open matching',

    'admin.roster.title': 'Roster',
    'admin.roster.lede': 'Everyone in the program. Status is computed from pairings, never stored.',
    'admin.roster.search': 'Search',
    'admin.roster.searchHint': 'Name, email, WeChat, school, subject',
    'admin.roster.role': 'Role',
    'admin.roster.all': 'All',
    'admin.roster.status': 'Status',
    'admin.roster.showing': '{shown} of {total}',
    'admin.roster.name': 'Name',
    'admin.roster.pairings': 'Pairings',
    'admin.roster.sessions': 'Classes',
    'admin.roster.none': 'Nobody matches that.',
    'admin.roster.addTutor': 'Add a tutor',
    'admin.roster.addStudent': 'Add a student',
    'admin.roster.added': 'Added {name}.',
    'admin.roster.newName': 'Name',
    'admin.roster.create': 'Add',
    'admin.roster.importCsv': 'Import roster CSV',
    'admin.roster.importHint': 'Adds new people and updates existing ones by id. Nothing is ever deleted by an import.',
    'admin.roster.imported': 'Added {added}, updated {updated}.',

    'admin.status.paired': 'Paired',
    'admin.status.unpaired': 'Unpaired',
    'admin.status.not-accepting': 'Not taking students',
    'admin.status.inactive': 'Inactive',

    'admin.person.back': 'Back to roster',
    'admin.person.details': 'Details',
    'admin.person.save': 'Save',
    'admin.person.saved': 'Saved.',
    'admin.person.remove': 'Remove',
    'admin.person.removed': 'Removed {name}.',
    'admin.person.removeConfirm': 'Remove {name} from the roster?',
    'admin.person.cannotRemove': 'Cannot remove',
    'admin.person.deactivate': 'Mark inactive',
    'admin.person.activate': 'Mark active',
    'admin.person.history': 'History',
    'admin.person.pairings': 'Pairings',
    'admin.person.sessions': 'Classes',
    'admin.person.noHistory': 'No history yet.',
    'admin.person.totals': '{sessions} classes · {hours}h',
    'admin.person.notFound': 'No such person.',
    'admin.person.field.name': 'Name',
    'admin.person.field.preferredName': 'Preferred name',
    'admin.person.field.email': 'Email',
    'admin.person.field.wechat': 'WeChat',
    'admin.person.field.school': 'School',
    'admin.person.field.grade': 'Grade',
    'admin.person.field.timezone': 'Time zone',
    'admin.person.field.subjects': 'Subjects',
    'admin.person.field.goals': 'Goals',
    'admin.person.field.interests': 'Interests',
    'admin.person.field.levels': 'Levels taken',
    'admin.person.field.englishLevel': 'English level',
    'admin.person.field.maxStudents': 'Maximum students',
    'admin.person.field.bio': 'Bio',
    'admin.person.field.meetingLink': 'Meeting link',
    'admin.person.listHint': 'Separate with semicolons',

    'admin.exp.title': 'Export',
    'admin.exp.lede': 'The program has to survive any individual leaving, including whoever built this.',
    'admin.exp.backup.title': 'Full backup',
    'admin.exp.backup.body': 'Everything, as one dated JSON file. This is the real save: it restores completely, and it is the only file anyone needs to keep.',
    'admin.exp.backup.action': 'Download backup',
    'admin.exp.reports.title': 'Spreadsheets',
    'admin.exp.reports.body': 'For anyone who wants the numbers in Excel. Roster, pairings and sessions round-trip back in; hours are a derived report and are export-only.',
    'admin.exp.tutors': 'Tutors',
    'admin.exp.students': 'Students',
    'admin.exp.pairings': 'Pairings',
    'admin.exp.sessions': 'Sessions',
    'admin.exp.availability': 'Availability',
    'admin.exp.hoursByTutor': 'Hours by tutor',
    'admin.exp.exportOnly': 'export only',
    'admin.exp.restore.title': 'Restore',
    'admin.exp.restore.body': 'Replaces everything currently loaded. Older backups are upgraded automatically, so a file from last term still opens.',
    'admin.exp.handover.title': 'Handing over',
    'admin.exp.handover.body': 'To pass the program on: send the JSON backup and the address of this page. Whoever takes over loads the file and has everything. There is no account to transfer and no server anybody has to keep paying for.',
    'error.render.title': 'This screen could not be drawn',
    'error.render.body': 'Your data is untouched. The export screen still works if you need a backup.',
    'busy.loading': 'Loading…',
    'busy.saving': 'Saving…',
    'busy.importing': 'Reading the file…',
    'busy.exporting': 'Preparing…',
    'busy.working': 'Working…',
    'brand.home': 'PeerBridges 2.0 — home',
    'lang.switch': 'Switch to Chinese',
    'tutor.log.lastTime': 'Last time: {text}',
    'tutor.log.coveredHint': 'One line is plenty. This is what you will read before the next class.',
    'sample.title': 'This is demo data.',
    'sample.body': 'Every name here is invented. Nothing on these screens is a real student, tutor or family.',
    'sample.replace': 'Load your own file',
    'sample.dismiss': 'This is real data',
    'sample.dismissConfirm': 'Only do this if the records loaded are genuinely your program\'s. The banner will stop appearing.',
    'sample.dismissed': 'Banner cleared. Remember to export a backup.',
    'toast.sampleLoaded': 'Sample data loaded.',
    'toast.imported': 'Imported {count} records.',
    'toast.exported': 'Saved {name}',
    'toast.cleared': 'This browser\'s copy has been cleared.',
    'toast.migrated': 'Migrated from schema {from} to {to}.',

    'placeholder.tag': 'Coming next',
    'placeholder.body': 'The shell, the router, and the data layer are in place. This screen is next.',
    'placeholder.willDo': 'What this screen will do',
    'placeholder.dataReady': 'Data already available to this screen:',

    'notfound.title': 'No such screen',
    'notfound.body': 'That link does not point anywhere in this app.',
    'notfound.action': 'Back to home',

    'action.loadSample': 'Load sample data',
    'action.goToData': 'Import a file',


    /* ── Class chat ─────────────────────────────────────────────── */
    'admin.exp.messages': 'Class messages',
    'settings.sessionLength': 'Default class length',
    'settings.zoneFixedTitle': 'The program clock is California',
    'settings.zoneFixed': 'Your time is always California, whatever clock this device happens to be set to. A coordinator on a school computer set to UTC, or reading this on a trip, must not have every date in the program quietly shift. Each tutor still keeps their own time zone, and students are always shown Beijing time.',
    'auth.title': 'Sign in',
    'auth.lede': 'to {program}',
    'auth.username': 'Username',
    'auth.usernameHint': 'The name the coordinator gave you.',
    'auth.password': 'Password or access code',
    'auth.passwordHint': 'Families: type the code you were sent. There is nothing to set up.',
    'auth.accessCode': 'Access code',
    'auth.signIn': 'Sign in',
    'auth.signingIn': 'Signing in…',
    'auth.signOut': 'Sign out',
    'auth.wrong': 'That username and password do not match. Check for a typo, or ask the coordinator.',
    'auth.forgot': 'Forgotten it? Ask the coordinator — they can issue you a new code in a few seconds. Nothing is lost.',
    'auth.notSecurityTitle': 'What signing in does, and does not do',
    'auth.notSecurity': 'Sign-in keeps people out of each other\'s screens on a shared device. It is not protection: this app has no server, so the whole program is stored in this browser and anyone who knows how can read it. Do not treat it as a lock on anybody\'s personal details.',
    'auth.noAccount': 'No account yet? Create one.',
    'auth.up.title': 'Create an account',
    'auth.up.lede': 'to join {program}',
    'auth.up.name': 'Your name',
    'auth.up.nameHint': 'So the coordinator knows who this is.',
    'auth.up.nameRequired': 'Please put your name, so the coordinator knows who is asking.',
    'auth.up.role': 'You are',
    'auth.up.roleHint': 'The coordinator will confirm this.',
    'auth.up.roleTutor': 'A tutor',
    'auth.up.roleStudent': 'A student',
    'auth.up.roleGuardian': 'A parent',
    'auth.up.usernameHint': 'Letters and numbers. You will type this to sign in.',
    'auth.up.passwordHint': 'At least 8 characters. A short phrase is easier to remember than a jumble.',
    'auth.up.submit': 'Create account',
    'auth.up.submitting': 'Creating…',
    'auth.up.haveAccount': 'Already have an account? Sign in.',
    'auth.up.approvalTitle': 'The coordinator confirms it first',
    'auth.up.approvalBody': 'Creating an account does not give you access straight away. The coordinator has to confirm who you are — until then you will not see any classes, students or contact details. This is how the program keeps children\'s details away from anyone who simply found the link.',
    'auth.pending.title': 'Waiting for the coordinator',
    'auth.pending.body': 'Your account for {program} is set up. The coordinator needs to confirm who you are before anything opens up — usually the same day.',
    'auth.pending.next': 'Nothing more for you to do. Sign in again later and everything will be here.',
    'auth.pending.nudge': 'In a hurry? Message the coordinator directly. Nothing is sent automatically.',
    'auth.admin.pending': 'Waiting to be confirmed',
    'auth.admin.pendingBody': 'These people created their own account. Say who each one is on the roster, and their screens open up. Until then they see nothing.',
    'auth.admin.pendingNone': 'Nobody is waiting.',
    'auth.admin.signedUpAs': 'signed up as',
    'auth.admin.linkTo': 'This is…',
    'auth.admin.confirm': 'Confirm',
    'auth.admin.confirmed': 'Confirmed. They can sign in now.',
    'auth.admin.reject': 'Turn off',
    'auth.admin.choosePerson': 'Choose a person',
    'auth.admin.pendingTag': 'Waiting',
    'auth.admin.title': 'Sign-in',
    'auth.admin.lede': 'Who can sign in, and how to give somebody access.',
    'auth.admin.firstTitle': 'Set up sign-in',
    'auth.admin.firstBody': 'Nobody signs in yet, and the app works without it. Create your own account first; you can then hand out access to everyone else.',
    'auth.admin.firstWarning': 'Write this password down somewhere safe before you close the tab. Nobody can reset it for you — there is no server and no email to send a reset to.',
    'auth.admin.createFirst': 'Create my account',
    'auth.admin.created': 'Account created.',
    'auth.admin.existing': 'Who can sign in',
    'auth.admin.invite': 'Give somebody access',
    'auth.admin.inviteBody': 'Generates a username and a short code to read out. Families never have to create anything.',
    'auth.admin.everyoneHas': 'Everybody on the roster already has access.',
    'auth.admin.giveAccess': 'Give access',
    'auth.admin.who': 'Who',
    'auth.admin.lastSignIn': 'Last signed in',
    'auth.admin.never': 'Never',
    'auth.admin.actions': 'Actions',
    'auth.admin.reissue': 'New code',
    'auth.admin.disable': 'Turn off',
    'auth.admin.enable': 'Turn on',
    'auth.admin.disabled': 'Off',
    'auth.admin.codeTitle': 'Send these two things',
    'auth.admin.codeBody': 'Give the person below their username and code — WeChat, email, or read it out.',
    'auth.admin.codeOnce': 'This code is shown once. It cannot be looked up again, but you can always issue a new one.',
    'auth.admin.copied': 'Done',
    'chat.nav': 'Messages',
    'chat.title': 'Messages',
    'chat.threadTitle': 'Class chat',
    'chat.subtitle': 'One conversation per class. Everyone in it sees everything.',
    'chat.empty': 'No classes to message about yet.',
    'chat.emptyTutor': 'Once you have a student, your class chat appears here.',
    'chat.emptyStudent': 'Once you have a tutor, you can message them here.',
    'chat.threadEmpty': 'Nothing here yet. Say hello.',
    'chat.threadEmptyHint': 'Anything about the class — a question, a time change, something you meant to send.',
    'chat.compose': 'Write a message',
    'chat.composeHint': 'Everyone in this class chat can read this, including the coordinator.',
    'chat.send': 'Send',
    'chat.sending': 'Sending…',
    'chat.unread': '{count} new',
    'chat.noMessagesYet': 'No messages yet',
    'chat.inThisChat': 'In this chat',
    'chat.you': 'You',
    'chat.roleTutor': 'Tutor',
    'chat.roleStudent': 'Student',
    'chat.roleGuardian': 'Parent',
    'chat.roleAdmin': 'Coordinator',
    'chat.guardianMaybe': 'Parent (if they join)',
    'chat.coordinatorAlways': 'The coordinator is in every class chat.',
    'chat.coordinatorWhy': 'This is a program for young people, so an adult can always see every conversation. There are no private messages and no way to switch this off.',
    'chat.openThread': 'Open chat',
    'chat.backToList': 'All messages',
    'chat.withdraw': 'Withdraw',
    'chat.withdrawn': 'This message was withdrawn.',
    'chat.withdrawConfirm': 'Withdraw this message? It will show as withdrawn rather than disappearing.',
    'chat.tooLong': 'That is longer than {max} characters. Try splitting it.',
    'chat.localOnlyTitle': 'Messages stay on this device',
    'chat.localOnly': 'This app has no server, so a message written here is saved in this browser only — the other people in this chat will not receive it. Treat it as your own notes for now, and keep using WeChat or email to actually reach someone.',
    'chat.localOnlyShort': 'Saved on this device only — not delivered.',
    'chat.paused': 'This class is paused.',
    'chat.classWith': 'Class with {name}',

  },

  zh: {
    'app.title': 'PeerBridges 2.0',
    'lang.toggle': 'English',

    'nav.home': '首页',
    'nav.tutors': '辅导员',
    'nav.students': '学生',
    'nav.sessions': '课程记录',
    'nav.hours': '志愿时长',
    'nav.settings': '设置',

    'footer.local': '所有数据都保存在此浏览器中。请导出以长期保存。',
    'footer.tests': '运行测试',

    'home.lede': '美国高中志愿者与中国大陆学生的一对一辅导。全部功能在浏览器中运行，没有服务器保存任何人的信息。',
    'home.empty.title': '尚未载入数据',
    'home.empty.body': '载入示例数据以了解本应用的用法，或导入此前导出的项目文件。',
    'home.zone.here': '你的时间',
    'home.zone.students': '学生时间',

    'students.title': '学生',
    'log.title': '记录课程',
    'hours.title': '志愿时长',
    'hours.lede': '由课程记录自动计算。无需任何人手动填写时长。',
    'settings.title': '设置',
    'settings.lede': '语言、时区与课程默认设置。',

    'data.import.drop': '选择一个 JSON 文件，或将文件拖到此处',
    'data.sample.action': '载入示例数据',
    'data.status.title': '当前载入',
    'data.status.schema': '数据结构版本',
    'data.status.cache': '浏览器缓存',
    'data.status.exported': '上次导出',
    'data.status.never': '从未',
    'data.reset.title': '重新开始',
    'data.reset.body': '清除此浏览器中的副本。已导出的文件不受影响。',
    'data.reset.action': '清除此浏览器的数据',
    'data.reset.confirm': '确定清除此浏览器中的所有数据吗？未导出的内容将无法恢复。',

    'data.status.pairings': '配对（进行中）',
    'data.status.availability': '可用时间条目',

    'selftest.title': '时区自检',
    'selftest.lede': '时区相关的断言，现在就在此浏览器中运行。这是本应用最容易出错的部分，因此也是你可以亲自核对的部分。',
    'selftest.pass': '全部 {total} 项检查通过',
    'selftest.fail': '{total} 项检查中有 {failed} 项未通过',
    'selftest.ranIn': '在此浏览器中运行耗时 {ms} 毫秒。',
    'selftest.rerun': '重新运行',
    'selftest.reran': '已重新运行检查。',
    'selftest.example': '最关键的例子',
    'selftest.tutorSide': '辅导员 · 美国太平洋时间',
    'selftest.studentSide': '学生 · 北京时间',
    'selftest.exampleNote': '同一时刻（{utc}），两端的称法不同。',
    'selftest.expected': '预期',
    'selftest.actual': '实际',
    'selftest.passed': '通过',
    'selftest.failed': '未通过',
    'selftest.nFailing': '{failed} 项未通过',
    'selftest.footnote': '这些断言与单元测试完全相同。完整测试请见',
    'footer.selftest': '时区自检',
    'role.label': '当前身份',
    'role.admin': '协调员',
    'role.tutors': '辅导员',
    'role.needTutor.title': '请先选择一位辅导员',
    'role.needTutor.body': '这些页面展示的是某一位辅导员自己的辅导情况。请在右上角选择你的身份。',
    'role.needTutor.action': '返回首页',

    'tutor.nav.home': '我的辅导',
    'tutor.nav.hours': '我的时长',
    'tutor.nav.availability': '我的可用时间',

    'tutor.home.greeting': '你好，{name}',
    'tutor.home.lede': '你的下一节课、你的学生，以及你的志愿时长。',
    'tutor.home.noStudents.title': '暂时还没有学生',
    'tutor.home.noStudents.body': '配对成功后，学生会显示在这里，同时还会显示你的下一节课以及你们一起学过的内容。',
    'tutor.home.setAvailability': '设置我的可用时间',

    'tutor.next.title': '下一节课',
    'tutor.next.none': '暂无安排。设置可用时间后，协调员就可以为你配对。',
    'tutor.next.yourTime': '你的时间',
    'tutor.next.theirTime': '对方时间',
    'tutor.next.recurring': '来自你们每周的共同时间段',
    'tutor.next.scheduled': '已安排',
    'tutor.next.join': '打开上课链接',
    'tutor.next.homework': '你上次布置的作业',
    'tutor.next.noHomework': '上次没有布置作业。',
    'tutor.next.lastCovered': '上次学习的内容',
    'tutor.next.logIt': '记录这节课',

    'tutor.students.title': '我的学生',
    'tutor.students.goals': '正在学习',
    'tutor.students.lastSession': '最近一次课',
    'tutor.students.never': '尚未开始',
    'tutor.students.sessions': '{count} 次课',
    'tutor.students.open': '打开学生页面',
    'tutor.students.log': '记录课程',

    'tutor.hours.title': '我的时长',
    'tutor.hours.lede': '根据你记录的课程自动计算。这里没有任何需要手动填写的数字。',
    'tutor.hours.basis': '每上一节课记 2 小时——包含备课、上课和课后跟进。不会问你花了多长时间，课上得短一些，记的时长也一样。',
    'tutor.hours.basisShort': '{count} 节课 × 2 小时',
    'tutor.hours.classTime': '上课时长',
    'tutor.hours.breakdownTitle': '时长是怎么算出来的',
    'tutor.cert.basis': '计算方式：本项目每上一节课统一记 {hours} 小时，包含备课、上课与课后跟进。总计 = {count} 节课 × {hours} 小时。不单独统计每节课的实际时长；下表中的每一节课均由志愿者在当时记录。',
    'tutor.hours.credited': '记入时长',
    'tutor.hours.thisTerm': '本学期',
    'tutor.hours.allTime': '累计',
    'tutor.hours.teaching': '授课',
    'tutor.hours.prep': '备课',
    'tutor.hours.followup': '课后',
    'tutor.hours.total': '合计',
    'tutor.hours.hoursShort': '小时',
    'tutor.hours.sessions': '课程次数',
    'tutor.hours.students': '学生人数',
    'tutor.hours.tableTitle': '全部课程',
    'tutor.hours.date': '日期',
    'tutor.hours.student': '学生',
    'tutor.hours.duration': '授课',
    'tutor.hours.empty': '尚未记录任何课程。记录之后就会显示在这里，并计入你的时长。',
    'tutor.hours.export.title': '导出用于时长认证',
    'tutor.hours.export.body': '一份可供签字的志愿时长记录，格式适用于 NHS、Congressional Award 与 President\'s Volunteer Service Award。可直接打印或另存为 PDF。',
    'tutor.hours.export.print': '打印／另存为 PDF',
    'tutor.hours.export.csv': '下载 CSV',
    'tutor.hours.export.term': '统计范围',
    'tutor.hours.export.allTime': '全部时间',

    'tutor.cert.title': '志愿服务时长记录',
    'tutor.cert.volunteer': '志愿者',
    'tutor.cert.organisation': '组织',
    'tutor.cert.activity': '服务内容',
    'tutor.cert.activityBody': '为中国大陆学生提供一对一线上学业辅导。课程在周末通过视频通话进行。',
    'tutor.cert.period': '统计期间',
    'tutor.cert.school': '学校',
    'tutor.cert.generated': '生成时间',
    'tutor.cert.summary': '时长汇总',
    'tutor.cert.detail': '课程明细',
    'tutor.cert.verify': '核实签字',
    'tutor.cert.verifyBody': '本人确认上述志愿者完成了本文件中记录的服务时长。',
    'tutor.cert.supervisor': '负责人姓名',
    'tutor.cert.signature': '签字',
    'tutor.cert.date': '日期',
    'tutor.cert.note': '时长由志愿者在每次课程当时记录的课程数据自动计算得出。',

    'tutor.log.title': '记录课程',
    'tutor.log.with': '与 {name}',
    'tutor.log.happened': '这节课上了吗？',
    'tutor.log.yes': '上了',
    'tutor.log.no': '没上',
    'tutor.log.duration': '上了多久？',
    'tutor.log.prep': '备课',
    'tutor.log.followup': '课后',
    'tutor.log.credits': '这节课记 2 小时。',
    'tutor.log.creditsNone': '这次没有课要记。这不会对你有任何影响。',
    'tutor.log.covered': '这节课学了什么？',
    'tutor.log.homework': '布置的作业',
    'tutor.log.optional': '选填',
    'tutor.log.save': '保存',
    'tutor.log.saved': '已保存，谢谢你。',
    'tutor.log.total': '合计 {total}',
    'tutor.log.capped': '每节课最多计 2 小时。',
    'tutor.log.minutes': '{n} 分钟',
    'tutor.log.none': '无',

    'tutor.nudge.title': '还没有记录',
    'tutor.nudge.body': '这些课已经过去了。方便的时候记录一下就好。',
    'tutor.nudge.log': '记录',

    'tutor.student.title': '学生',
    'tutor.student.about': '基本情况',
    'tutor.student.level': '英语水平',
    'tutor.student.goals': '学习目标',
    'tutor.student.interests': '兴趣爱好',
    'tutor.student.grade': '年级',
    'tutor.student.timezone': '时区',
    'tutor.student.pairedSince': '配对开始于',
    'tutor.student.handover': '从这里接手',
    'tutor.student.handoverBody': '本页的写法是为了让新接手的人读完就知道该从哪里继续。',
    'tutor.student.history': '课程记录',
    'tutor.student.covered': '学习内容',
    'tutor.student.homework': '作业',
    'tutor.student.didNotHappen': '未进行',
    'tutor.student.noHistory': '尚未记录任何课程。',
    'tutor.student.notYours': '这不是你的学生。',
    'tutor.student.totals': '共 {sessions} 次课 · {hours} 小时',

    'tutor.avail.title': '我的可用时间',
    'tutor.avail.lede': '仅在你希望接收新学生时使用。这不是每周都必须有空的承诺。',
    'tutor.avail.accepting': '愿意接收新学生',
    'tutor.avail.acceptingOn': '你目前愿意与新学生配对。',
    'tutor.avail.acceptingOff': '你目前不接收新学生。其他一切不变，现有学生不受影响。',
    'tutor.avail.windows': '周末时间段',
    'tutor.avail.windowsBody': '按你的当地时间填写，同时显示中国学生一侧对应的时间。',
    'tutor.avail.add': '添加时间段',
    'tutor.avail.remove': '删除',
    'tutor.avail.day': '星期',
    'tutor.avail.from': '从',
    'tutor.avail.to': '到',
    'tutor.avail.theirs': '中国时间',
    'tutor.avail.none': '尚未设置时间段。添加之后，协调员就能为你配对合适的学生。',
    'tutor.avail.save': '保存可用时间',
    'tutor.avail.saved': '可用时间已保存。',
    'tutor.avail.overnight': '跨越午夜',

    'weekday.0': '星期日',
    'weekday.1': '星期一',
    'weekday.2': '星期二',
    'weekday.3': '星期三',
    'weekday.4': '星期四',
    'weekday.5': '星期五',
    'weekday.6': '星期六',
    'role.students': '学生',
    'role.guardians': '家长',
    'role.guardianOf': '{name} 的家长',
    'role.needStudent.title': '请先选择一位学生',
    'role.needStudent.body': '本页面展示某一位学生自己的课程。请在右上角选择你的身份。',

    'st.nav.home': '我的课程',
    'st.title': '我的课程',
    'st.greeting': '你好，{name}',
    'st.tutor.title': '我的辅导员',
    'st.tutor.teaches': '教授科目',
    'st.tutor.contact': '联系方式',
    'st.tutor.none': '还没有为你配对辅导员。协调员正在安排。',
    'st.next.title': '下一节课',
    'st.next.none': '尚未安排课程。',
    'st.next.join': '进入课堂',
    'st.next.tutorTime': '辅导员的时间',
    'st.homework.title': '作业',
    'st.homework.none': '目前没有作业。',
    'st.homework.from': '{date} 布置',
    'st.history.title': '往期课程',
    'st.history.homework': '作业',
    'st.history.none': '还没有上过课。',
    'st.history.didNotHappen': '这节课没有进行',
    'st.nothingRequired': '本页面无需你填写任何内容。',

    'gd.contact.title': '家长联系方式',
    'gd.contact.body': '均为选填。留空即可，项目运作完全不受影响。',
    'gd.contact.name': '你的姓名',
    'gd.contact.wechat': '微信',
    'gd.contact.email': '电子邮箱',
    'gd.contact.save': '保存',
    'gd.contact.saved': '已保存。',
    'gd.contact.onlyYou': '只有你可以修改这些内容。',

    'match.nav': '配对',
    'match.title': '配对',
    'match.lede': '这些只是建议。在你确认之前，不会建立任何配对。',
    'match.waiting.title': '等待配对的学生',
    'match.waiting.none': '所有在读学生都已有辅导员。',
    'match.blocked.title': '暂时无人可以接收这些学生',
    'match.blocked.body': '每一位都会说明需要改变什么。',
    'match.idle.title': '有名额但无合适学生的志愿者',
    'match.idle.body': '这些辅导员有时间也愿意接收。值得为他们招募合适的学生。',
    'match.stale.title': '已无共同时间的配对',
    'match.stale.body': '这些配对仍在进行，但双方的可用时间已经错开。没有人会主动提起，因此这里是唯一能发现的地方。',
    'match.stale.noOverlap': '没有共同时间',
    'match.stale.missing': '其中一方尚未填写可用时间',
    'match.candidate': '建议 {n}',
    'match.why': '推荐理由',
    'match.watch': '需要留意',
    'match.score': '匹配度',
    'match.accept': '建立配对',
    'match.accepted': '已将 {student} 与 {tutor} 配对。',
    'match.slotsOpen': '{total} 个名额中还有 {remaining} 个',
    'match.nearest': '目前最接近的',
    'match.blockedBy': '受阻于',
    'match.counts': '{waiting} 人等待 · {matchable} 人可配对 · {blocked} 人受阻',

    'match.part.morning': '上午',
    'match.part.afternoon': '下午',
    'match.part.evening': '晚上',

    'match.reason.overlap': '共有 {hours} 小时重叠时间，北京时间{weekday}{part}（{startTime}–{endTime}）',
    'match.reason.goals': '双方都填写了{list}',
    'match.reason.level': '辅导员可以教{level}水平',
    'match.reason.interests': '双方都对{list}感兴趣',
    'match.reason.capacity': '辅导员 {total} 个名额中还有 {remaining} 个',

    'match.weak.single-window': '只有一个共同时间段——任何一方有事就很难调整',
    'match.weak.short-overlap': '共同时间只有 {minutes} 分钟，几乎没有调整空间',
    'match.weak.goals-partial': '无法覆盖{list}',
    'match.weak.level-unstated': '水平是否合适尚未确认——没有人填写可教的水平范围',
    'match.weak.no-shared-interests': '没有共同的兴趣爱好',
    'match.weak.no-shared-language': '没有共同语言，而学生是初学者',
    'match.weak.tutor-last-slot': '这是该辅导员的最后一个名额',

    'match.blocker.no-shared-time': '没有共同时间',
    'match.blocker.no-shared-goal': '不教该学生想学的科目',
    'match.blocker.level-outside-comfort': '水平超出该辅导员可教范围',
    'match.blocker.tutor-at-capacity': '辅导员名额已满',
    'match.blocker.tutor-not-accepting': '辅导员目前不接收新学生',
    'match.blocker.tutor-inactive': '辅导员正在休息',
    'match.blocker.student-inactive': '该学生未在读',

    'match.fix.no-tutors': '名单中还没有辅导员。',
    'match.fix.matchable': '这位学生可以配对。',
    'match.fix.one-tutor-full': '有一位辅导员非常合适，但已没有名额。这需要再招一位同科目、同时段的辅导员，而不是继续询问。',
    'match.fix.all-tutors-full': '所有合适的辅导员都已达到自己设定的上限。这需要再招一位辅导员。',
    'match.fix.no-shared-time': '没有人与这位学生有共同的周末时段。可以请学生再填一个时段，或招募更靠西部时区的辅导员。',
    'match.fix.no-shared-goal': '没有可用的辅导员教这位学生想学的科目。可以针对该科目招募，或问问学生还有什么需要。',
    'match.fix.level-outside-comfort': '没有可用的辅导员填写可教这一英语水平。值得问问是否有人愿意尝试。',
    'match.fix.tutor-not-accepting': '合适的辅导员目前不接收新学生。',
    'match.fix.tutor-inactive': '合适的辅导员正在休息。',
    'match.fix.student-inactive': '该学生已标记为未在读。',
    'admin.nav.overview': '总览',
    'admin.nav.attention': '需要关注',
    'admin.nav.roster': '名单',
    'admin.nav.export': '导出',

    'admin.computed': '这些页面上的所有数据都由大家本来就在记录的内容自动计算得出。不会为了让这里出现一个数字而要求任何人填表。',

    'admin.ov.title': '总览',
    'admin.ov.lede': '项目当前的整体情况。',
    'admin.ov.activePairings': '进行中的配对',
    'admin.ov.tutors': '辅导员',
    'admin.ov.students': '学生',
    'admin.ov.paused': '{n} 个暂停',
    'admin.ov.inactive': '{n} 人未在读',
    'admin.ov.waiting': '{n} 人等待配对',
    'admin.ov.hours': '授课时长',
    'admin.ov.week': '本周',
    'admin.ov.month': '本月',
    'admin.ov.allTime': '累计',
    'admin.ov.growth': '增长情况',
    'admin.ov.growthBody': '每月末的名单人数，以及当月进行的课程数。',
    'admin.ov.sessionsPerMonth': '已进行课程',
    'admin.ov.attentionCta': '查看需要关注的事项',

    'admin.att.title': '需要关注',
    'admin.att.lede': '这些情况没有人会主动上报。',
    'admin.att.allClear': '目前没有需要关注的事项。所有进行中的配对近期都上过课，每位学生都有辅导员，本月也没有取消的课程。',
    'admin.att.quiet.title': '已经沉寂的配对',
    'admin.att.quiet.body': '已有 {weeks} 周以上没有课程记录，沉寂最久的排在前面。适合发一条友善的消息问问近况。',
    'admin.att.quiet.days': '{n} 天',
    'admin.att.quiet.never': '尚未开始上课',
    'admin.att.quiet.lastMet': '最近一次 {date}',
    'admin.att.quiet.none': '所有进行中的配对近期都上过课。',
    'admin.att.waiting.title': '还没有辅导员的学生',
    'admin.att.waiting.body': '在读但没有进行中配对的学生。',
    'admin.att.waiting.none': '每位在读学生都有辅导员。',
    'admin.att.waiting.returning': '此前有过辅导员',
    'admin.att.waiting.new': '新学生',
    'admin.att.capacity.title': '仍有名额的辅导员',
    'admin.att.capacity.body': '愿意接收且尚未满员。闲置的志愿者不会抱怨，只会慢慢离开。',
    'admin.att.capacity.none': '目前没有辅导员有空余名额。',
    'admin.att.capacity.open': '{capacity} 个名额中还有 {remaining} 个',
    'admin.att.misses.title': '没有进行的课程',
    'admin.att.misses.body': '最近 {days} 天。偶尔一次很正常；连续多次通常说明情况有变。',
    'admin.att.misses.none': '最近 {days} 天没有取消的课程。',
    'admin.att.contact': '联系方式',
    'admin.att.noContact': '没有留存联系方式',
    'admin.att.openMatching': '打开配对页面',

    'admin.roster.title': '名单',
    'admin.roster.lede': '项目中的所有人。状态由配对情况自动计算，不做存储。',
    'admin.roster.search': '搜索',
    'admin.roster.searchHint': '姓名、邮箱、微信、学校、科目',
    'admin.roster.role': '身份',
    'admin.roster.all': '全部',
    'admin.roster.status': '状态',
    'admin.roster.showing': '共 {total} 人，显示 {shown} 人',
    'admin.roster.name': '姓名',
    'admin.roster.pairings': '配对',
    'admin.roster.sessions': '课程',
    'admin.roster.none': '没有符合条件的人。',
    'admin.roster.addTutor': '添加辅导员',
    'admin.roster.addStudent': '添加学生',
    'admin.roster.added': '已添加 {name}。',
    'admin.roster.newName': '姓名',
    'admin.roster.create': '添加',
    'admin.roster.importCsv': '导入名单 CSV',
    'admin.roster.importHint': '新增人员并按 id 更新已有记录。导入操作绝不会删除任何数据。',
    'admin.roster.imported': '新增 {added} 人，更新 {updated} 人。',

    'admin.status.paired': '已配对',
    'admin.status.unpaired': '未配对',
    'admin.status.not-accepting': '不接收新学生',
    'admin.status.inactive': '未在读',

    'admin.person.back': '返回名单',
    'admin.person.details': '详细信息',
    'admin.person.save': '保存',
    'admin.person.saved': '已保存。',
    'admin.person.remove': '删除',
    'admin.person.removed': '已删除 {name}。',
    'admin.person.removeConfirm': '确定将 {name} 从名单中删除吗？',
    'admin.person.cannotRemove': '无法删除',
    'admin.person.deactivate': '标记为未在读',
    'admin.person.activate': '标记为在读',
    'admin.person.history': '历史记录',
    'admin.person.pairings': '配对记录',
    'admin.person.sessions': '课程记录',
    'admin.person.noHistory': '暂无历史记录。',
    'admin.person.totals': '{sessions} 次课 · {hours} 小时',
    'admin.person.notFound': '找不到此人。',
    'admin.person.field.name': '姓名',
    'admin.person.field.preferredName': '常用称呼',
    'admin.person.field.email': '电子邮箱',
    'admin.person.field.wechat': '微信',
    'admin.person.field.school': '学校',
    'admin.person.field.grade': '年级',
    'admin.person.field.timezone': '时区',
    'admin.person.field.subjects': '可教科目',
    'admin.person.field.goals': '学习目标',
    'admin.person.field.interests': '兴趣爱好',
    'admin.person.field.levels': '可教水平',
    'admin.person.field.englishLevel': '英语水平',
    'admin.person.field.maxStudents': '学生数上限',
    'admin.person.field.bio': '个人简介',
    'admin.person.field.meetingLink': '上课链接',
    'admin.person.listHint': '用分号分隔',

    'admin.exp.title': '导出',
    'admin.exp.lede': '即使任何一个人离开，项目也必须能延续下去——包括做出这套系统的人。',
    'admin.exp.backup.title': '完整备份',
    'admin.exp.backup.body': '全部数据，导出为一个带日期的 JSON 文件。这才是真正的存档：可以完整还原，也是唯一需要保存的文件。',
    'admin.exp.backup.action': '下载备份',
    'admin.exp.reports.title': '表格文件',
    'admin.exp.reports.body': '供需要在 Excel 中查看数据的人使用。名单、配对与课程可以重新导入；时长属于统计报表，仅支持导出。',
    'admin.exp.tutors': '辅导员',
    'admin.exp.students': '学生',
    'admin.exp.pairings': '配对',
    'admin.exp.sessions': '课程',
    'admin.exp.availability': '可用时间',
    'admin.exp.hoursByTutor': '各辅导员时长',
    'admin.exp.exportOnly': '仅导出',
    'admin.exp.restore.title': '恢复数据',
    'admin.exp.restore.body': '将替换当前载入的全部数据。较旧的备份会自动升级，上学期的文件同样可以打开。',
    'admin.exp.handover.title': '交接',
    'admin.exp.handover.body': '交接项目只需两件事：把 JSON 备份文件和本页面的网址发给接手的人。对方载入文件即可获得全部数据。没有账号需要转移，也没有服务器需要有人继续付费。',
    'error.render.title': '此页面无法显示',
    'error.render.body': '你的数据未受影响。如需备份，导出页面仍可正常使用。',
    'busy.loading': '载入中…',
    'busy.saving': '保存中…',
    'busy.importing': '正在读取文件…',
    'busy.exporting': '正在准备…',
    'busy.working': '处理中…',
    'brand.home': 'PeerBridges 2.0 — 首页',
    'lang.switch': '切换为英文',
    'tutor.log.lastTime': '上次：{text}',
    'tutor.log.coveredHint': '一行就够。下次上课前你会先看到这句话。',
    'sample.title': '这是演示数据。',
    'sample.body': '此处所有姓名均为虚构。这些页面上没有任何真实的学生、辅导员或家庭。',
    'sample.replace': '载入你自己的文件',
    'sample.dismiss': '这是真实数据',
    'sample.dismissConfirm': '只有当前载入的确实是你项目的真实记录时才这样做。此提示将不再出现。',
    'sample.dismissed': '提示已关闭。请记得导出备份。',
    'toast.sampleLoaded': '示例数据已载入。',
    'toast.imported': '已导入 {count} 条记录。',
    'toast.exported': '已保存 {name}',
    'toast.cleared': '此浏览器中的副本已清除。',
    'toast.migrated': '数据结构已从版本 {from} 升级到 {to}。',

    'placeholder.tag': '即将推出',
    'placeholder.body': '应用框架、路由与数据层已就绪。此页面为下一步开发内容。',
    'placeholder.willDo': '此页面将实现',
    'placeholder.dataReady': '此页面已可使用的数据：',

    'notfound.title': '页面不存在',
    'notfound.body': '该链接在本应用中没有对应的页面。',
    'notfound.action': '返回首页',

    'action.loadSample': '载入示例数据',
    'action.goToData': '导入文件',


    /* ── 班级群聊 ────────────────────────────────────────────────── */
    'admin.exp.messages': '班级消息',
    'settings.sessionLength': '默认课时长度',
    'settings.zoneFixedTitle': '项目时间以加州为准',
    'settings.zoneFixed': '无论这台设备的时区设置成什么，"你的时间"始终是加州时间。负责老师如果用学校的电脑（时区可能是 UTC），或者在外地打开，项目里的日期都不应该悄悄发生变化。每位辅导老师仍然有自己的时区，学生看到的始终是北京时间。',
    'auth.title': '登录',
    'auth.lede': '进入 {program}',
    'auth.username': '用户名',
    'auth.usernameHint': '负责老师给你的名字。',
    'auth.password': '密码或访问码',
    'auth.passwordHint': '家长：直接输入收到的访问码即可，不需要注册。',
    'auth.accessCode': '访问码',
    'auth.signIn': '登录',
    'auth.signingIn': '登录中…',
    'auth.signOut': '退出',
    'auth.wrong': '用户名和密码不匹配。请检查是否输错，或联系负责老师。',
    'auth.forgot': '忘记了？联系负责老师，几秒钟就能重新发一个新的访问码，不会丢失任何内容。',
    'auth.notSecurityTitle': '登录能做什么，不能做什么',
    'auth.notSecurity': '登录只是让共用同一台设备的人不会看到彼此的界面。它不是保护措施：这个应用没有服务器，整个项目的数据都存在这个浏览器里，懂技术的人都能看到。请不要把它当成对个人信息的保护。',
    'auth.noAccount': '还没有账号？创建一个。',
    'auth.up.title': '创建账号',
    'auth.up.lede': '加入 {program}',
    'auth.up.name': '你的名字',
    'auth.up.nameHint': '方便负责老师知道这是谁。',
    'auth.up.nameRequired': '请填写你的名字，方便负责老师知道是谁申请的。',
    'auth.up.role': '你的身份',
    'auth.up.roleHint': '负责老师会确认这一项。',
    'auth.up.roleTutor': '辅导老师',
    'auth.up.roleStudent': '学生',
    'auth.up.roleGuardian': '家长',
    'auth.up.usernameHint': '字母和数字。登录时要输入这个。',
    'auth.up.passwordHint': '至少 8 个字符。一句短语比一串乱码更容易记住。',
    'auth.up.submit': '创建账号',
    'auth.up.submitting': '创建中…',
    'auth.up.haveAccount': '已经有账号了？去登录。',
    'auth.up.approvalTitle': '需要负责老师先确认',
    'auth.up.approvalBody': '创建账号后不会立刻开通。负责老师需要先确认你的身份——在那之前，你看不到任何课程、学生或联系方式。这样可以确保孩子们的信息不会被只是拿到链接的人看到。',
    'auth.pending.title': '等待负责老师确认',
    'auth.pending.body': '你在 {program} 的账号已经创建好了。负责老师确认你的身份之后就会开通，通常当天就能处理。',
    'auth.pending.next': '你这边不需要再做什么。稍后再登录，内容就都在了。',
    'auth.pending.nudge': '着急的话，可以直接联系负责老师。系统不会自动发送任何消息。',
    'auth.admin.pending': '等待确认',
    'auth.admin.pendingBody': '这些人自己创建了账号。指出他们在名单上是哪一位，页面就会为他们开通。在此之前他们什么也看不到。',
    'auth.admin.pendingNone': '没有人在等待。',
    'auth.admin.signedUpAs': '注册身份为',
    'auth.admin.linkTo': '这位是…',
    'auth.admin.confirm': '确认',
    'auth.admin.confirmed': '已确认，对方现在可以登录了。',
    'auth.admin.reject': '停用',
    'auth.admin.choosePerson': '选择一位',
    'auth.admin.pendingTag': '等待中',
    'auth.admin.title': '登录管理',
    'auth.admin.lede': '谁可以登录，以及如何给别人开通。',
    'auth.admin.firstTitle': '设置登录',
    'auth.admin.firstBody': '目前还没有人需要登录，不设置也能正常使用。先创建你自己的账号，然后就可以给其他人开通。',
    'auth.admin.firstWarning': '关闭页面前请把这个密码记在安全的地方。没有人能帮你重置——这里没有服务器，也没有可以发送重置邮件的地址。',
    'auth.admin.createFirst': '创建我的账号',
    'auth.admin.created': '账号已创建。',
    'auth.admin.existing': '可以登录的人',
    'auth.admin.invite': '给别人开通',
    'auth.admin.inviteBody': '会生成一个用户名和一串简短的访问码，方便口头告知。家长不需要注册任何东西。',
    'auth.admin.everyoneHas': '名单上的每个人都已经可以登录了。',
    'auth.admin.giveAccess': '开通',
    'auth.admin.who': '成员',
    'auth.admin.lastSignIn': '最近登录',
    'auth.admin.never': '从未',
    'auth.admin.actions': '操作',
    'auth.admin.reissue': '换新码',
    'auth.admin.disable': '停用',
    'auth.admin.enable': '启用',
    'auth.admin.disabled': '已停用',
    'auth.admin.codeTitle': '把这两项发给对方',
    'auth.admin.codeBody': '把下面的用户名和访问码告诉对方——微信、邮件或口头都可以。',
    'auth.admin.codeOnce': '访问码只显示这一次，之后无法再查看，但随时可以重新生成一个。',
    'auth.admin.copied': '完成',
    'chat.nav': '消息',
    'chat.title': '消息',
    'chat.threadTitle': '班级群聊',
    'chat.subtitle': '每个班一个群聊。群里的每个人都能看到全部内容。',
    'chat.empty': '暂时还没有可以聊天的班级。',
    'chat.emptyTutor': '有学生之后，班级群聊会出现在这里。',
    'chat.emptyStudent': '配好老师之后，就可以在这里给老师发消息。',
    'chat.threadEmpty': '这里还没有消息。打个招呼吧。',
    'chat.threadEmptyHint': '和上课有关的都可以说——一个问题、改时间，或者想发的材料。',
    'chat.compose': '写消息',
    'chat.composeHint': '这个班级群聊里的每个人都能看到，包括负责老师。',
    'chat.send': '发送',
    'chat.sending': '发送中…',
    'chat.unread': '{count} 条新消息',
    'chat.noMessagesYet': '还没有消息',
    'chat.inThisChat': '群聊成员',
    'chat.you': '你',
    'chat.roleTutor': '辅导老师',
    'chat.roleStudent': '学生',
    'chat.roleGuardian': '家长',
    'chat.roleAdmin': '负责老师',
    'chat.guardianMaybe': '家长（如果加入的话）',
    'chat.coordinatorAlways': '负责老师在每一个班级群聊里。',
    'chat.coordinatorWhy': '这是面向青少年的项目，所以始终有一位成年人能看到全部对话。没有私聊，也无法关闭这一点。',
    'chat.openThread': '打开群聊',
    'chat.backToList': '全部消息',
    'chat.withdraw': '撤回',
    'chat.withdrawn': '这条消息已撤回。',
    'chat.withdrawConfirm': '撤回这条消息吗？它会显示为已撤回，而不是直接消失。',
    'chat.tooLong': '超过了 {max} 个字符，建议分成几条发。',
    'chat.localOnlyTitle': '消息只保存在这台设备上',
    'chat.localOnly': '这个应用没有服务器，所以在这里写的消息只保存在这个浏览器里——群聊里的其他人收不到。目前请把它当作自己的备忘，真正要联系人还是用微信或邮件。',
    'chat.localOnlyShort': '仅保存在本设备，尚未送达。',
    'chat.paused': '这个班暂停中。',
    'chat.classWith': '与 {name} 的班级',

  }
};

const missingWarned = new Set();

let current = storedLang() ?? browserLang();

/** The language this browser has explicitly chosen, or null. */
export function storedLang() {
  const saved = loadLangPreference();
  return LANGS.includes(saved) ? saved : null;
}

/** True once somebody has actually used the toggle. */
export function hasExplicitLang() {
  return storedLang() !== null;
}

function browserLang() {
  const nav = (globalThis.navigator?.languages ?? [globalThis.navigator?.language ?? 'en'])
    .join(',')
    .toLowerCase();
  return nav.includes('zh') ? 'zh' : 'en';
}

/**
 * The language a role should start in.
 *
 * Students and guardians get Chinese by default: they are the people least
 * likely to read English, and asking a family to hunt for a toggle before
 * they can read their own child's homework is the opposite of being served.
 * An explicit choice always wins over this.
 */
export function defaultLangFor(role) {
  return role === 'student' || role === 'guardian' ? 'zh' : browserLang();
}

export function getLang() {
  return current;
}

export function getLocale() {
  return LOCALES[current];
}

/** Switch language for this render. Does not persist — see chooseLang. */
export function setLang(lang) {
  if (!LANGS.includes(lang)) return current;
  current = lang;
  // Guarded so the dictionaries can be checked outside a browser.
  if (typeof document !== 'undefined') {
    document.documentElement.lang = lang === 'zh' ? 'zh-Hans' : 'en';
  }
  return current;
}

/** Switch language and remember it, overriding any role default from here on. */
export function chooseLang(lang) {
  if (!LANGS.includes(lang)) return current;
  saveLangPreference(lang);
  return setLang(lang);
}

export function toggleLang() {
  return chooseLang(current === 'en' ? 'zh' : 'en');
}

/**
 * Look up a key. `vars` fills {placeholders}.
 * @param {string} key
 * @param {Record<string, string|number>} [vars]
 */
export function t(key, vars) {
  let str = STRINGS[current]?.[key];

  if (str === undefined) {
    str = STRINGS.en[key];
    if (str === undefined) {
      if (!missingWarned.has(key)) {
        missingWarned.add(key);
        console.warn(`[i18n] no string for "${key}"`);
      }
      return key;
    }
  }

  if (!vars) return str;
  return str.replace(/\{(\w+)\}/g, (whole, name) =>
    Object.prototype.hasOwnProperty.call(vars, name) ? String(vars[name]) : whole
  );
}

/**
 * Every key defined for a language.
 *
 * Exists so the test suite can compare the two dictionaries directly rather
 * than against a hand-kept list — a list is the thing that drifts, and a key
 * missing from Chinese is exactly the failure principle 6 is about.
 */
export function allKeys(lang) {
  return Object.keys(STRINGS[lang] ?? {}).sort();
}

/** The raw string for a key in a specific language, without fallback. */
export function rawString(lang, key) {
  return STRINGS[lang]?.[key];
}

/** Apply translations to any element carrying data-i18n. */
export function applyStaticStrings(root = document) {
  for (const el of root.querySelectorAll('[data-i18n]')) {
    el.textContent = t(el.dataset.i18n);
  }
  for (const el of root.querySelectorAll('[data-i18n-label]')) {
    el.setAttribute('aria-label', t(el.dataset.i18nLabel));
  }
}
